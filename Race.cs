...
namespace RaceResults
{
    class Race
    {
        ...
        static public int addRace(string filename, category cat)
        {
            ...
            race.id = stevec;
            race.name = filename;
            var value = race.name.Split('_');
            race.country = value[1];

            if (int.TryParse(value[2].Split('.')[0],out int leto))
            {
                race.year = leto;
            }
            else
            {
                foreach (var beseda in value)
                {
                    if (int.TryParse(beseda.Split('.')[0],out int Leto))
                    {
                        race.year = Leto;
                    }
